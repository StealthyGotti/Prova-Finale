...
% Tempo di trasferimento dal punto di rilascio all'apocentro (dove effettuare il cambio di forma)
dt1 = flightTime (a_i, e_i, th_i, 180);
TableCompiler( [kepEli(1:5) 180] , 0 , dt1 , 'F1PA')
hold on
...
[dv_2, wf, thf, dt3] = changeOrbPlane (a_f, e_f, i_i, OM_i, om_i, i_f, OM_f, 0);
TableCompiler( [kepElf(1:2) kepEli(3:5) thf] , 0 , dt1+dt2+dt3, 0)
TableCompiler( [ kepElf(1:4) wf thf], dv_2 , dt1+dt2+dt3, 0)
plotOrbitQuiver([a_f , e_f , i_i , OM_i , om_i , 0 ], mu, 5 , thf)  % tratto 3

hold on

% CAMBIO ANOMALIA
[dv_3, thm ,thf1, dt4] = changePerArg (a_f , e_f, wf , om_f , thf);
TableCompiler( [ kepElf(1:5) thm], dv_3 , dt1+dt2+dt3+dt4)
plotOrbitQuiver([a_f , e_f , i_f , OM_f , wf , thf ], mu, 5 , thm)  % tratto 4
...
TableCompiler( [ kepElf(1:6)], 0 , dt1+dt2+dt3+dt4+dt5)
...
legend('Init Pos', 'Fin Pos', 'Orb Init','','Shape Changed','', 'Plane Changed' ,'', 'Periapsis Arg Changed' ,'', 'Orb Fin')

title('F1PA procedure')

% COSTO TOTALE E TEMPO DI TRASFERIMENTO DELLA MISSIONE

dv_TOT = abs(dv_1) + abs(dv_2) + abs(dv_3);   % 3.6834 km/s  % più costoso rispetto a APF e AFP
dt_TOT_sec = dt1 + dt2 + dt3 + dt4 + dt5; % 22392 sec = 6 hr 13 min 12 s
...
plotOrbitQuiver([a_i , e_i , i_i , OM_i , om_i , th_i ], mu, 5 , 0)  % tratto 1

% Tempo di trasferimento dal punto di rilascio all'apocentro (dove effettuare il cambio di forma)
dt1 = flightTime (a_i, e_i, th_i, 0);
TableCompiler( [kepEli(1:5) 180] , 0 , dt1 , 'F2AP')

hold on

% CAMBIO FORMA 2  (pericentro orbita iniziale -> apocentro orbita finale)

[dvi, dvf, th, dt2, a_t, e_t] = biTangent (a_i, e_i, a_f, e_f, 0);
dv_1 = dvi + dvf;
TableCompiler( [a_t e_t kepEli(3:5) 180] , dvi , dt1, 0)
TableCompiler( [kepElf(1:2) kepEli(3:5) 0] , dvf , dt1+dt2, 0)

plotOrbitQuiver([a_t , e_t , i_i , OM_i , om_i , 0 ], mu, 5 , 180)  % tratto 2
...
[dv_2, thm ,thf1, dt3] = changePerArg (a_f , e_f, om_i , om_st , 180);
TableCompiler(  [kepElf(1:2) kepEli(3:5) thm] , 0 , dt1+dt2+dt3, 0 )
TableCompiler(  [kepElf(1:2) kepEli(3:4) om_st thf1] , dv_2 , dt1+dt2+dt3, 0 )
...
% CAMBIO PIANO
[dv_3, wf, thf, dt4] = changeOrbPlane (a_f, e_f, i_i, OM_i, om_st, i_f, OM_f, thf1);
TableCompiler( [kepElf(1:2) kepEli(3:4) om_st thf] , 0 , dt1+dt2+dt3+dt4, 0 )
TableCompiler( [kepElf(1:2) kepElf(3:5) thf] , dv_3 , dt1+dt2+dt3+dt4, 0 )
...
title('F2AP procedure')

% COSTO TOTALE E TEMPO DI TRASFERIMENTO DELLA MISSIONE

dv_TOT = abs(dv_1) + abs(dv_2) + abs(dv_3);   % 3.4932 km/s ~= 3.49 km/s
dt_TOT_sec = dt1 + dt2 + dt3 + dt4 + dt5; % 25280 sec = 7 hr 1 min 20 s

[h, m, s] = time2esa(dt_TOT_sec);

fprintf('%cv F2AP: %.2f km/s\n', 916, dv_TOT)
fprintf('%ct F2AP: %.0f s = %.0f hr %.0f min %.0f s\n', 916, dt_TOT_sec, h, m, s)
...
dt1 = flightTime (a_i, e_i, th_i, 0);
TableCompiler( [kepEli(1:5) 180] , 0 , dt1 , 'F2PA')
...
[dv_2, wf, thf, dt3] = changeOrbPlane (a_f, e_f, i_i, OM_i, om_i, i_f, OM_f, 180);
TableCompiler( [kepElf(1:2) kepEli(3:5) thf] , 0 , dt1+dt2+dt3, 0)
TableCompiler(  [kepElf(1:4) wf thf] , dv_2 , dt1+dt2+dt3, 0)

plotOrbitQuiver([a_f , e_f , i_i , OM_i , om_i , 180 ], mu, 5 , thf)  % tratto 3

hold on

% CAMBIO ANOMALIA
[dv_3, thm ,thf1, dt4] = changePerArg (a_f , e_f, wf , om_f , thf);
TableCompiler([kepElf(1:4) wf thm] , 0 , dt1+dt2+dt3+dt4, 0)
TableCompiler([kepElf(1:5) thf1] , dv_3 , dt1+dt2+dt3+dt4, 0)
...
title('F2PA procedure')

% COSTO TOTALE E TEMPO DI TRASFERIMENTO DELLA MISSIONE

dv_TOT = abs(dv_1) + abs(dv_2) + abs(dv_3);   % 3.6770 km/s  % più costoso rispetto a APF e AFP, ma leggermente meno costoso di F1PA
dt_TOT_sec = dt1 + dt2 + dt3 + dt4 + dt5; % 22119 sec = 6 hr 8 min 39 s
[h, m, s] = time2esa(dt_TOT_sec);

fprintf('%cv F2PA: %.2f km/s\n', 916, dv_TOT)
fprintf('%ct F2PA: %.0f s = %.0f hr %.0f min %.0f s\n', 916, dt_TOT_sec, h, m, s)
...
% cambio piano -> cambio forma (1) -> cambio anomalia
...
% CAMBIO PIANO
[dv_1, wf, thf, dt1] = changeOrbPlane (a_i, e_i, i_i, OM_i, om_i, i_f, OM_f, th_i);
TableCompiler( [kepEli(1:5) thf] , 0 , dt1 , 'PF1A')
TableCompiler( [ kepEli(1:2) kepElf(3:4) wf thf] , dv_1 , dt1, 0)
...
plotOrbitQuiver([a_i , e_i , i_f , OM_f , wf , thf ], mu, 5 , 180 )  % tratto 2: trasferimento dal punto di cambio piano all'apocentro

% Tempo di trasferimento dal punto di cambio piano all'apocentro
dt2 = flightTime (a_i, e_i, thf, 180);
TableCompiler ( [ kepEli(1:2) kepElf(3:4) wf 180] , 0 , dt1+dt2 , 0 )

% CAMBIO FORMA (apocentro orbita iniziale -> pericentro orbita finale)
[dvi, dvf, th, dt3, a_t, e_t] = biTangent (a_i, e_i, a_f, e_f, 180);
dv_2 = dvi + dvf;
TableCompiler( [ a_t e_t kepElf(3:4) wf 180], dvi , dt1+dt2)
TableCompiler( [ kepElf(1:4) wf 0], dvf , dt1+dt2+dt3)

plotOrbitQuiver([a_t , e_t , i_f , OM_f , wf , 180 ], mu, 5 , 0 )  % tratto 3

hold on

% CAMBIO ANOMALIA
[dv_3, thm ,thf1, dt4 ] = changePerArg (a_f, e_f, wf , om_f , 0 );
TableCompiler( [ kepElf(1:4) wf thm], 0 , dt1+dt2+dt3+dt4)
TableCompiler( [ kepElf(1:4) wf thf1], dv_3 , dt1+dt2+dt3+dt4)

plotOrbitQuiver([a_f , e_f , i_f , OM_f , wf , 0 ], mu, 5 , thm)  % tratto 4
...
title('PF1A procedure')

% COSTO TOTALE E TEMPO DI TRASFERIMENTO DELLA MISSIONE

dv_TOT = abs(dv_1) + abs(dv_2) + abs(dv_3);   % 3.7630 km/s ~= 3.76 km/s
dt_TOT_sec = dt1 + dt2 + dt3 + dt4 + dt5; % 23189 sec = 5 hr 8 min 38 s
[h, m, s] = time2esa(dt_TOT_sec);

fprintf('%cv PF1A: %.2f km/s\n', 916, dv_TOT)
fprintf('%ct PF1A: %.0f s = %.0f hr %.0f min %.0f s\n', 916, dt_TOT_sec, h, m, s)
...
% CAMBIO PIANO
[dv_1, wf, thf, dt1] = changeOrbPlane (a_i, e_i, i_i, OM_i, om_i, i_f, OM_f, th_i);
TableCompiler( [kepEli(1:5) thf] , 0 , dt1 , 'PF2A')
TableCompiler( [ kepEli(1:2) kepElf(3:4) wf thf] , dv_1 , dt1, 0)
...
dv_2 = dvi + dvf;
TableCompiler( [ a_t e_t kepElf(3:4) wf 0], dvi , dt1+dt2)
TableCompiler( [ kepElf(1:4) wf 180], dvf , dt1+dt2+dt3)
...
fprintf('%cv PF2A: %.2f km/s\n', 916, dv_TOT)
fprintf('%ct PF2A: %.0f s = %.0f hr %.0f min %.0f s\n', 916, dt_TOT_sec, h, m, s)
%% MANOVRA BIELLITTICA
