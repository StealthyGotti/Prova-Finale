...
i_ir = rad2deg(i_i);
OM_ir = rad2deg(OM_i);
om_st = om_fr - (144.1217 - om_fr); % anomalia del perigeo "strategico", il quale permette di ottenere
...in seguito al cambio di piano (nel seguente punto di manovra) l'anomalia di perigeo richiesta per l'orbita finale.

[dv_1, thm ,thf1, dt_1 ] = changePerArg (a_i, e_i, om_ir , om_st , th_ir);
%[delta_T] = timeOfFlight (a_i,e_i,th_ir,thm);

plotOrbitQuiver([a_i , e_i , i_ir , OM_ir , om_ir , th_ir ], mu, 20, thm)  % tratto 1
hold on
...
[dv_2, wf , thf2, dt2] = changeOrbPlane (a_i, e_i, i_ir, OM_ir, om_st, i_fr, OM_fr, thf1);

plotOrbitQuiver([a_i , e_i , i_ir , OM_ir , om_st , thf1 ], mu, 5 , thf2)  % tratto 2
hold on
plotOrbitQuiver([a_i , e_i , i_fr , OM_fr , wf , thf2 ], mu, 2 , 180 )  % tratto 3
...
plotOrbitQuiver([a_t1 , e_t1 , i_fr , OM_fr , wf , 180 ], mu, 20, 0) % tratto 4  % da rivedere

hold on

theta_fr = rad2deg(theta_f);

plotOrbitQuiver([a_f , e_f , i_fr , OM_fr , wf , 0 ], mu, 5, theta_fr) % tratto 5

% Tempo di trasferimento dal perigeo della nuova orbita al punto d'arrivo (theta_f)
dt5 = flightTime (a_f, e_f, 0 , theta_fr);
